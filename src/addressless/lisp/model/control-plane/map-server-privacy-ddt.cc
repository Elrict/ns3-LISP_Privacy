#include "map-server-privacy-ddt.h"
...
namespace ns3
{
	NS_LOG_COMPONENT_DEFINE("MapServerPrivacyDdt");

	NS_OBJECT_ENSURE_REGISTERED(MapServerPrivacyDdt);

	TypeId
	MapServerPrivacyDdt::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::MapServerPrivacyDdt")
								.SetParent<MapServer>()
								.SetGroupName("Lisp")
								.AddConstructor<MapServerPrivacyDdt>();
		return tid;
	}

	Ptr<RandomVariableStream>
	MapServerPrivacyDdt::GetMdsModel(void)
	{
	...
	MapServerPrivacyDdt::MapServerPrivacyDdt()
	{
		/**
		 * MapServerPrivacyDdt and LispOverIp should point to the same MapTables!
		 */
	...
	Ptr<MapTables>
	MapServerPrivacyDdt::GetMapTablesV4()
	{
		return m_mapTablesv4;
	}

	Ptr<MapTables>
	MapServerPrivacyDdt::GetMapTablesV6()
	{
		return m_mapTablesv6;
	}

	void
	MapServerPrivacyDdt::SetMapTables(Ptr<MapTables> mapTablesV4,
	...
	void
	MapServerPrivacyDdt::StartApplication(void)
	{
		NS_LOG_FUNCTION(this);
		...
		m_socket->SetRecvCallback(MakeCallback(&MapServerPrivacyDdt::HandleRead, this));
		...
		m_msClientSocket6->SetRecvCallback(
			MakeCallback(&MapServerPrivacyDdt::HandleReadFromClient, this));
		m_msClientSocket->SetRecvCallback(
			MakeCallback(&MapServerPrivacyDdt::HandleReadFromClient, this));
	}

	void
	MapServerPrivacyDdt::StopApplication(void)
	{
		NS_LOG_FUNCTION(this);
	...
	Ptr<MapNotifyMsg>
	MapServerPrivacyDdt::GenerateMapNotifyMsg(Ptr<MapRegisterMsg> msg)
	{
	...
	void
	MapServerPrivacyDdt::Send(Ptr<Packet> p)
	{
		NS_LOG_FUNCTION(this);
	...
	void
	MapServerPrivacyDdt::HandleRead(Ptr<Socket> socket)
	{
	}

	void
	MapServerPrivacyDdt::PopulateDatabase(Ptr<MapRegisterMsg> msg)
	{
		...
		if (record->GetEidAfi() == LispControlMsg::IP)
		{
			...
			mapEntry->SetProxyReply(msg->GetP());
			/**
	...
	void
	MapServerPrivacyDdt::HandleReadFromClient(Ptr<Socket> socket)
	{
		NS_LOG_FUNCTION(this);
		Ptr<Packet> packet;
		Address from;
		...
		while ((packet = socket->RecvFrom(from)))
		{
			...
			if (msg_type == static_cast<uint8_t>(MapRegisterMsg::GetMsgType()))
			{
				...
				MapServerPrivacyDdt::PopulateDatabase(msg);
			...
			else if (msg_type == static_cast<uint8_t>(MapRequestMsg::GetMsgType()))
			{
				...
				if (entry == 0)
				{
					...
					Simulator::Schedule(Seconds(m_searchTimeVariable->GetValue()), &MapServerPrivacyDdt::Send, this, reactedPacket);
				}
				else
				{
					...
					if (entry->GetProxyReply() == 1)
					{
						...
						Simulator::Schedule(Seconds(m_searchTimeVariable->GetValue()), &MapServerPrivacyDdt::ForwardRequest, this, requestMsg->GetItrRlocAddrIp(), reactedPacket);
					}
					else
					{
						Simulator::Schedule(Seconds(m_searchTimeVariable->GetValue()), &MapServerPrivacyDdt::ForwardRequest, this, locator->GetRlocAddress(), reqPacket);
					}
				}
			}
			else if (msg_type == static_cast<uint8_t>(InfoRequestMsg::GetMsgType()))
			{
	...
	void
	MapServerPrivacyDdt::ForwardRequest(Address peerAddress, Ptr<Packet> p)
	{

		MapResolver::ConnectToPeerAddress(peerAddress,
										  m_peerPort, m_socket);
		Send(p);
	}

	Ptr<MapReplyMsg>
	MapServerPrivacyDdt::GenerateNegMapReply(Ptr<MapRequestMsg> requestMsg)
	{
	...
	Ptr<InfoRequestMsg>
	MapServerPrivacyDdt::GenerateInfoReplyMsg(Ptr<InfoRequestMsg> msg, uint16_t etrUdpPort, Address etrAddress, Address msAddress)
	{
