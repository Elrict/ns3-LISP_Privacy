...
#define TAP_MAGIC 95549
  ...
  TapBridge::CreateTap(void)
  {
    ...
    if (pid == 0)
    {
      ...
      if (wantIp && (ipv4 == 0) && m_tapIp.IsBroadcast() && m_tapNetmask.IsEqual(Ipv4Mask::GetOnes()))
      {
      ...
      //
      NS_FATAL_ERROR("TapBridge::CreateTap(): Back from execlp(), status = " << status << " errno = " << ::strerror(errno));
    }
    else
    {
