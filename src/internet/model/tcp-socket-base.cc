...
namespace ns3
{
  ...
  TcpSocketBase::ProcessAck(const SequenceNumber32 &ackNumber, bool scoreboardUpdated)
  {
    ...
    if (scoreboardUpdated ||
        (!m_sackEnabled && ackNumber == m_txBuffer->HeadSequence() && ackNumber < m_tcb->m_nextTxSequence))
    {
      NS_LOG_DEBUG("ACK of " << ackNumber << " SND.UNA=" << m_txBuffer->HeadSequence() << " SND.NXT=" << m_tcb->m_nextTxSequence << " we are ready to process the dupAck");
      // loss recovery check is done inside this function thanks to
      // the congestion state machine
      DupAck();
    }

    if (ackNumber == m_txBuffer->HeadSequence() && ackNumber == m_tcb->m_nextTxSequence)
    {
      NS_LOG_INFO("ACK of " << ackNumber << ", there is no need to process (we haven't data to transmit)");
      // Dupack, but the ACK is precisely equal to the nextTxSequence
      return;
    }
    else if (ackNumber == m_txBuffer->HeadSequence() && ackNumber > m_tcb->m_nextTxSequence)
    {
    ...
    else if (ackNumber > m_txBuffer->HeadSequence())
    {
      ...
      else if (ackNumber < m_recover && m_tcb->m_congState == TcpSocketState::CA_LOSS)
      {
        ...
        NS_LOG_DEBUG("Ack of " << ackNumber << ", equivalent of " << segsAcked << " segments in CA_LOSS. Cong Control Called, cWnd=" << m_tcb->m_cWnd << " ssTh=" << m_tcb->m_ssThresh);
      ...
      else
      {
        if (m_tcb->m_congState == TcpSocketState::CA_OPEN)
        {
          NS_LOG_DEBUG(segsAcked << " segments acked in CA_OPEN, ack of " << ackNumber);
          m_congestionControl->PktsAcked(m_tcb, segsAcked, m_lastRtt);
        }
        else if (m_tcb->m_congState == TcpSocketState::CA_DISORDER)
        {
  ...
  TcpSocketBase::AdvertisedWindowSize(bool scale) const
  {
    ...
    uint32_t w = (m_rxBuffer->MaxRxSequence() > m_rxBuffer->NextRxSequence()) ? m_rxBuffer->MaxRxSequence() - m_rxBuffer->NextRxSequence() : 0;
